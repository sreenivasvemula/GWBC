package gw.webservice.bc.maintenanceTools;
...
uses gw.api.webservice.maintenanceTools.MaintenanceToolsBase
...
class IMaintenanceToolsAPI {
  ...
  /**
   * Imports sample data by calling a method on the SampleData.DataSets Gosu class
   * @param dataSet
   */
  @Throws(RequiredFieldException, "If the required field dataSet is null.")
  @Throws(PermissionException, "If the user does not have SOAP ADMIN permissions")
  @Throws(DataConversionException, "If there is an issue while generating sample data.")
  public function importSampleData(dataSet : String) {
    if (dataSet == null) {
      throw new RequiredFieldException("Required field dataSet may not be null.")
    }
    try {
      //var instance = new SampleData.DataSets()
      var instance = eval("new SampleData.DataSets()")
      SampleDataGenerator.generateSampleData(instance, dataSet)
    } catch (cnfe : ClassNotFoundException) {
      throw new DataConversionException(cnfe.getMessage())
    } catch (nsme : NoSuchMethodException) {
      throw new DataConversionException(nsme.getMessage())
    }
  }

  /**
   * Generates the default Gosu sample data, as configured in config.xml
   */
  @Throws(PermissionException, "If the user does not have SOAP ADMIN permissions")
  @Throws(DataConversionException, "If there is an issue while generating sample data.")
  function importDefaultSampleData() {
    try {
   ...
   * @return The status of the queue
   *
   * @deprecated (Since 8.0.0) Use the method {@link #getWQueueStatus(java.lang.String)} instead.  It returns more detailed and accurate
   * information about a worker queue executors and workers.
   */
  @java.lang.Deprecated
  @Throws(PermissionException, "")
  @Throws(SOAPServerException, "")
  function getWorkQueueStatus(queueName : String) : WQueueStatus {
    return getMaintenanceImpl().getWQueueStatus( queueName )
  }
  ...
  private function getMaintenanceImpl() : BCMaintenanceToolsImpl {
    return new BCMaintenanceToolsImpl()
  }
}
