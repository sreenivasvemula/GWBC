 ...
 */
@WsiWebService("http://guidewire.com/bc/ws/gw/webservice/bc/MessagingToolsAPI")
...
class MessagingToolsAPI {
  ...
  /**
   * Resync the Account against a specified destination
   *
   * @param destID The destination against which the Account should be resynced.
   * @param accountPublicID The publicID of the Account that should be resynced.
   */
  @Throws(RequiredFieldException, "If accountPublicID is null")
  @Throws(BadIdentifierException, "If account cannot be found")
  @Throws(WsiAuthenticationException, "if there are permission/authentication issues")
  function resyncAccount(destID : int, accountPublicID : String) {
    WebservicePreconditions.notNull(accountPublicID, "accountPublicID")
    getDelegate().resyncAccount( destID, accountPublicID )
  }

  /**
   * Resync the Producer against a specified destination
   *
   * @param destID The destination against which the Producer should be resynced.
   * @param producerPublicID The publicID of the Producer that should be resynced.
   */
  @Throws(RequiredFieldException, "If producerPublicID is null")
  @Throws(BadIdentifierException, "If producer cannot be found")
  @Throws(WsiAuthenticationException, "if there are permission/authentication issues")
  function resyncProducer(destID : int, producerPublicID : String) {
    WebservicePreconditions.notNull(producerPublicID, "producerPublicID")
    getDelegate().resyncProducer( destID, producerPublicID )
  }

  /**
   * Resync the PolicyPeriod against a specified destination
   *
   * @param destID The destination against which the PolicyPeriod should be resynced.
   * @param policyPeriodPublicID The publicID of the PolicyPeriod that should be resynced.
   */
  @Throws(RequiredFieldException, "If policyPeriodPublicID is null")
  @Throws(BadIdentifierException, "If the PolicyPeriod cannot be found")
  @Throws(WsiAuthenticationException, "if there are permission/authentication issues")
  function resyncPolicyPeriod(destID : int, policyPeriodPublicID : String) {
    WebservicePreconditions.notNull(policyPeriodPublicID, "policyPeriodPublicID")
    getDelegate().resyncPolicyPeriod( destID, policyPeriodPublicID )
  }
  ...
  private function getDelegate() : gw.api.webservice.bc.messagingTools.BCMessagingToolsImpl {
    return new gw.api.webservice.bc.messagingTools.BCMessagingToolsImpl()
  }
}
