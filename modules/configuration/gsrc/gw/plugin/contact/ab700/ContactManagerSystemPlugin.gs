...
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.anonymous.elements.AddressBookUIDContainer_AddressBookUIDTuples_Entry
...
uses java.util.Set
/*
  @deprecated (since 8.0.0) Please use gw.plugin.contact.ab800.ABContactSystemPlugin instead
*/
@java.lang.Deprecated
@Export
class ContactManagerSystemPlugin implements ContactSystemPlugin {
   ...
   * is successsful, this method updates the contact, primary address and secondary addresses with their AddressBookUIDs.
   ...
   * an activity and also tries to retrive the latest version of the contact.
   *
   * @param contact contact that ContactManager is being notified about.
  ...
  override function retrieveContact(addressBookUID : String, creator : ContactCreator) : Contact {
    ...
    if (contactXml != null) {
      var contactType = ContactIntegrationXMLMapper.getInstance().getNameMapper().getLocalEntityName(contactXml.EntityType)
  ...
  private function findContactsFromContactManager(abSearchCriteria : ABContactSearchCriteriaInfo,
                                                  searchSpec : ABContactAPISearchSpecInfo) : ContactResultFromSearch[] {
  ...
  private function throwExceptionIfContactManagerReturnsTooManyResults(abSearchCriteria : ABContactSearchCriteriaInfo,
                                                                       searchSpec : ABContactAPISearchSpecInfo)  {
  ...
  private function populateAddressBookUIDField(addressBookUIDEntry : AddressBookUIDContainer_AddressBookUIDTuples_Entry,
    ...
    var localEntityName = ContactIntegrationXMLMapper.getInstance().getNameMapper().getLocalEntityName(addressBookUIDEntry.EntityType)
    ...
    if (bean == null) {
      bean = contact.Bundle.loadByPublicId(entityType, addressBookUIDEntry.External_PublicID)
    }
  ...
  protected function handleErrors(e : Exception) {
    if (e typeis IllegalArgumentException or
      ...
      throw e  // Let it through for other types of exception
    }
  }
